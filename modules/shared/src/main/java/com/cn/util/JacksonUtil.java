...
//            configCommon();
    ...
    private static void configPropertyInclusion() {
        objectMapper.setDefaultPropertyInclusion(DEFAULT_PROPERTY_INCLUSION);
        yamlMapper.setDefaultPropertyInclusion(DEFAULT_PROPERTY_INCLUSION);
    }
    ...
    public static <V> V toCollection(URL url, TypeReference<V> type) {
        try {
            return objectMapper.readValue(url.openStream(), type);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static <T> void add(JsonNode jsonNode, String key, T value) {
        switch (value) {
            case String str -> ((ObjectNode) jsonNode).put(key, str);
            case Short vst -> ((ObjectNode) jsonNode).put(key, vst);
            case Integer vnt -> ((ObjectNode) jsonNode).put(key, vnt);
            case Long vlt -> ((ObjectNode) jsonNode).put(key, vlt);
            case Float vft -> ((ObjectNode) jsonNode).put(key, vft);
            case Double vde -> ((ObjectNode) jsonNode).put(key, vde);
            case BigDecimal vbd -> ((ObjectNode) jsonNode).put(key, vbd);
            case BigInteger vbt -> ((ObjectNode) jsonNode).put(key, vbt);
            case Boolean vbl -> ((ObjectNode) jsonNode).put(key, vbl);
            case byte[] vbe -> ((ObjectNode) jsonNode).put(key, vbe);
            case null, default -> ((ObjectNode) jsonNode).put(key, stringify(value));
        }
    }
